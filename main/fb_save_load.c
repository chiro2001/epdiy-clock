...
esp_err_t fb_save_compressed() {
    ...
    esp_err_t r;
    if (ESP_OK != (r = compress_mem_to_file(filename_fb_compressed_front, hl.front_fb, fb_size, FRAME_COMPRESS_LEVEL))) {
        ESP_LOGE(TAG, "compress_mem_to_file front failed!");
        return r;
    }
    if (ESP_OK != (r = compress_mem_to_file(filename_fb_compressed_back, hl.back_fb, fb_size, FRAME_COMPRESS_LEVEL))) {
        ESP_LOGE(TAG, "compress_mem_to_file back failed!");
        return r;
    }
    if (ESP_OK != (r = compress_mem_to_file(filename_fb_compressed_diff, hl.difference_fb, fb_size * 2, FRAME_COMPRESS_LEVEL))) {
